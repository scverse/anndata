...
- {attr}`anndata.settings.auto_shard_zarr_v3` and `anndata.settings.zarr_write_format` are no longer dependent on each other, as stated in the docs {user}`ilan-gold` ({pr}`2427`)
