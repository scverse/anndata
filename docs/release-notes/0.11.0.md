...
* Add ability to convert strings to categoricals on write in {func}`anndata.write_h5ad` and {func}`anndata.write_zarr` via `convert_strings_to_categoricals` parameter {pr}`1474` {user}` falexwolf`
