...
- {guilabel}`rc1` Move `RWAble` and `InMemoryElem` out of `experimental`, renaming `RWAble` to `AxisStorable`[^storable] and `InMemoryElem` to {type}`~anndata.typing.RWAble` {user}`ilan-gold` ({pr}`1643`)
...
[^storable]: `anndata.typing.AxisStorable` was later again renamed to {type}`~anndata.typing.Storable`
