...
* Public API for interacting with on disk sparse arrays: {func}`~anndata.sparse_dataset`, {class}`~anndata.CSRDataset`, and {class}`~anndata.CSCDataset` {pr}`765` {user}`ilan-gold` {user}`ivirshup`
