...
- {guilabel}`rc4` Use {attr}`numpy.dtypes.StringDType` with `na_object` set to {arr}`pandas.NA` for nullable string data with {class}`anndata.experimental.backed.Dataset2D` {user}`ilan-gold` ({pr}`2011`)
