...
def equal_sparse(a, b) -> bool:
    ...
    if isinstance(b, CupySparseMatrix | sparse.spmatrix | CSArray):
...
def as_sparse(x, use_sparse_array=False):
    if not isinstance(x, sparse.spmatrix | CSArray):
...
class Reindexer:
    ...
    def apply(self, el, *, axis, fill_value=None):
        ...
        elif isinstance(el, sparse.spmatrix | CSArray | CupySparseMatrix):
...
def default_fill_value(els):
    ...
    """
    if any(
        isinstance(el, sparse.spmatrix | CSArray)
        or (
            isinstance(el, DaskArray)
            and isinstance(el._meta, sparse.spmatrix | CSArray)
        )
        for el in els
    ):
        return 0
...
def concat_arrays(arrays, reindexers, axis=0, index=None, fill_value=None):
    ...
    elif any(isinstance(a, sparse.spmatrix | CSArray) for a in arrays):
