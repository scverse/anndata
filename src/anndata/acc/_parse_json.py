...
def as_idx_2d(data: Sequence[str | int | None]) -> Idx2D | None:
    if (
        len(data) != 2
        or not all(isinstance(i, str | None) for i in data)
...
def parse_json[R: AdRef](a: AdAcc[R], data: Sequence[str | int | None]) -> R:
    match data:
        case ["l", str() | None as layer, *idx] if idx := as_idx_2d(idx):
        ...
        case ["op" | "vp" as ax, str(k), *idx] if idx := as_idx_2d(idx):
